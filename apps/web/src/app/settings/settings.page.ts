  ...
  template: `
    <div class="mx-auto grid w-full max-w-5xl items-start gap-6 lg:grid-cols-[13rem_minmax(0,1fr)]">
      <div>
        ...
        <h1 class="mt-1 font-display text-2xl font-bold tracking-tight dark:text-zinc-50">Settings</h1>
        <nav class="mt-4 flex gap-1 overflow-x-auto lg:flex-col" aria-label="Settings">
          @for (item of sections; track item.id) {
            <button
              ...
              [class]="section() === item.id ? 'bg-[#f7f7f4] font-semibold text-[#121417] dark:bg-zinc-800 dark:text-zinc-50' : 'text-[#63676c] hover:bg-[#f7f7f4] dark:text-zinc-400 dark:hover:bg-zinc-800'"
            >{{ item.label }}</button>
          }
        </nav>
      </div>
      <div>
        ...
        <section class="rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <form class="mt-4 grid gap-3 sm:grid-cols-2" (ngSubmit)="saveStudio()">
            ...
            <div class="sm:col-span-2">
              <button type="submit" [class]="btn">Save studio</button>
            </div>
          </form>
        </section>
          }
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div class="space-y-3">
            ...
            <button type="button" (click)="addSig()" [class]="btn">Add Signature</button>
          </div>
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div class="space-y-3">
            ...
            <button type="button" (click)="addSet()" [class]="btn">Create Posting Set</button>
          </div>
          <div class="space-y-2">
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div class="space-y-3">
            ...
            <button type="button" (click)="addGroup()" [class]="btn">Add Customer Group</button>
          </div>
          <div class="space-y-3">
            @for (g of groups(); track g.id) {
              <div class="rounded-xl border border-[#e8e8e3] bg-[#f7f7f4] p-3 dark:border-zinc-700 dark:bg-zinc-800">
                ...
                <button type="button" (click)="saveGroupAccounts(g.id)" [class]="btn">Save assignments</button>
              </div>
            }
          </div>
        </section>
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div class="space-y-3">
            ...
            <button type="button" (click)="addRss()" [class]="btn">Watch Feed</button>
          </div>
          <div class="space-y-2">
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button type="button" (click)="addPlug('internal')" [class]="btn">Add internal</button>
              <button type="button" (click)="addPlug('global')" [class]="btn">Add global</button>
            </div>
          </div>
          <div class="space-y-2">
        ...
        <section class="flex flex-col space-y-6 rounded-2xl border border-[#e8e8e3] bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
          <div class="flex items-center justify-between gap-3">
            ...
            <button type="button" (click)="createToken()" [class]="btn">Create Token</button>
          </div>
          @if (newToken()) {
          ...
          <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <label class="block text-xs font-medium text-zinc-700 md:col-span-2 dark:text-zinc-400">Webhook Endpoint URL
              <div class="mt-1.5 flex flex-col gap-2 sm:flex-row">
                ...
                <button type="button" (click)="addHook()" [class]="btn">Add Webhook</button>
              </div>
            </label>
          </div>
          <div class="space-y-2">
...
export class SettingsPage implements OnInit {
  ...
  readonly btn = "inline-flex h-8 shrink-0 items-center rounded-full bg-cta px-3 text-[11px] font-semibold text-white hover:bg-cta-hover";
